...
# class MSMLikelihoodNetwork(nn.Module):
#     def __init__(self, config):
...
class VisualEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.vis_embeddings = nn.Embedding.from_pretrained(config.imagenet_embeddings, max_norm=5.0, freeze=True)
...
class OntologicalEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.conceptnet_embeddings = nn.Embedding.from_pretrained(config.conceptnet_embeddings, max_norm=5.0,
                                                                  freeze=True)
...
class LinguisticsEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.histwords_embeddings = nn.Embedding.from_pretrained(config.histwords_embeddings, max_norm=5.0, freeze=True)
